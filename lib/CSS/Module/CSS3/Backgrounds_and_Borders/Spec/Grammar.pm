...
grammar CSS::Module::CSS3::Backgrounds_and_Borders::Spec::Grammar {
    ...
    rule expr-border-top {:i :my @*SEEN; [ <expr-border-width> <!seen(0)> | <expr-border-style> <!seen(1)> | <color> <!seen(2)> ]+ }

    #| border-right: <'border-width'> || <'border-style'> || <color>
    rule decl:sym<border-right> {:i (border\-right) ':' <val( rx{ <expr=.expr-border-right> }, &?ROUTINE.WHY)> }
    rule expr-border-right {:i :my @*SEEN; [ <expr-border-width> <!seen(0)> | <expr-border-style> <!seen(1)> | <color> <!seen(2)> ]+ }

    #| border-bottom: <'border-width'> || <'border-style'> || <color>
    rule decl:sym<border-bottom> {:i (border\-bottom) ':' <val( rx{ <expr=.expr-border-bottom> }, &?ROUTINE.WHY)> }
    rule expr-border-bottom {:i :my @*SEEN; [ <expr-border-width> <!seen(0)> | <expr-border-style> <!seen(1)> | <color> <!seen(2)> ]+ }

    #| border-left: <'border-width'> || <'border-style'> || <color>
    rule decl:sym<border-left> {:i (border\-left) ':' <val( rx{ <expr=.expr-border-left> }, &?ROUTINE.WHY)> }
    rule expr-border-left {:i :my @*SEEN; [ <expr-border-width> <!seen(0)> | <expr-border-style> <!seen(1)> | <color> <!seen(2)> ]+ }
